...
// https://github.com/Schneegans/schneegans.github.io/blob/master/_posts/2015-09-20-signal-slot.md

// A signal object may call multiple slots with the same signature. You can connect functions to the signal which will be
// called when the emit() method on the signal object is invoked. Any argument passed to emit() will be passed to the given
// functions.
...
class Signal {
 public:
  ...
  void emit(Args... p) {
    for (auto const& it : _slots) { it.second(p...); }
  }
  ...
  void emit_for_all_but_one(int excludedConnectionID, Args... p) {
    for (auto const& it : _slots) {
      if (it.first != excludedConnectionID) { it.second(p...); }
    }
  }
